...
select * from movies left join users on movies.movie_id = users.favorite_movie_id;
...
SELECT movies.lead_studio, COUNT(users.user_id) AS favorite_movie_count FROM movies INNER JOIN users ON movies.movie_id = users.favorite_movie_id GROUP BY movies.lead_studio ORDER BY favorite_movie_count DESC LIMIT 1;

-- <<<<<<<<<<<<<<<<<<<<<< PROBLEM 5 >>>>>>>>>>>>>>>>>>>>>>>
-- Get the average Rotten Tomatoes score of all movies that are favorited by a user.
-- EXPECTED RESULT: 52.21
SELECT AVG(movies.rotten_tomatoes) AS rotten_tomatoes FROM movies INNER JOIN users ON movies.movie_id = users.favorite_movie_id;
...
SELECT film FROM movies GROUP BY film HAVING COUNT(*) > 1 LIMIT 2;
